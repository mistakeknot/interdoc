...
#
# Usage:
#   scripts/bump-version.sh 4.6.0
#   scripts/bump-version.sh 4.6.0 --dry-run
...
usage() {
    ...
    echo "  version   Semver string, e.g. 4.6.0"
...
# Update marketplace entry (matches by plugin name context)
MARKETPLACE_CURRENT=$(grep -A10 '"interdoc"' "$MARKETPLACE_ROOT/.claude-plugin/marketplace.json" | grep '"version"' | sed 's/.*"\([0-9][^"]*\)".*/\1/')
update_file \
    "$MARKETPLACE_ROOT/.claude-plugin/marketplace.json" \
    "\"version\": \"$MARKETPLACE_CURRENT\"" \
    "\"version\": \"$VERSION\"" \
    "interagency-marketplace/marketplace.json (interdoc entry)"
...
git push
echo -e "${GREEN}Pushed interdoc${NC}"

cd "$MARKETPLACE_ROOT"
git add .claude-plugin/marketplace.json
git commit -m "chore: bump interdoc to v$VERSION"
git push
echo -e "${GREEN}Pushed interagency-marketplace${NC}"

echo ""
echo -e "${GREEN}Done!${NC} interdoc v$VERSION"
echo ""
